...
const pillars = [
  { icon: <BatteryCharging size={20} />, title: 'Electric composure', copy: 'Fast charging, calm torque delivery, and route-aware range logic for daily confidence.' },
            ...
            >
              <div className="flex h-full flex-col justify-between p-5 md:p-8">
                <div className="flex flex-wrap items-center gap-3">
                  <span className="rounded-full border border-white/15 bg-black/35 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-sky-200 backdrop-blur-xl">M8 Competition</span>
                  <span className="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs text-zinc-200 backdrop-blur-xl">Studio render loop</span>
                </div>
                <div className="max-w-lg rounded-[1.7rem] border border-white/10 bg-black/40 p-5 backdrop-blur-2xl">
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-sky-300">Showcase loop</p>
                  <h3 className="mt-3 font-display text-3xl font-semibold tracking-[-0.04em] text-white md:text-5xl">Light reveals the surface.</h3>
                </div>
              </div>
            </RenderVideo>
          </motion.div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            {pillars.map((pillar, index) => (
              <motion.article
                ...
                className="rounded-3xl border border-white/10 bg-white/[0.045] p-5 shadow-panel backdrop-blur-2xl"
              >
